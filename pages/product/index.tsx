...
const Index = () => {
  const [prices, setPrices] = useState<any>([10, 20, 30]);
  const [price, setPrice] = useState<any>(prices[0]);
  const [size, setSize] = useState<any>(0);
  const [extraItems, setExtraItems] = useState<any>(itemsExtra);
  ...
  const handleChange = (e: any, item: any) => {
    ...
    if (checked) {
      ...
      changePrice(-item.price);
      setExtras(extras.filter((extra: any) => extra?.id !== item?.id));
    }
  };
